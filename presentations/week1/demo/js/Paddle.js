...
function getIntersectPoint(speedX, speedY, posX, posY, interY) {
	var result = {x: 0, y: 0};
	var m = speedX/speedY;
	result.y = interY;
	result.x = posX + m*(result.y - posY);
	if (result.x < 0) {
		result.x = 0;
		result.y = (result.x - posX)/m + posY;
...
function moveComputerPaddle() {
	...
	if (ballVelY > 0) {
		...
		// if (paddleX + PADDLE_W/2 > canvas.width + 40) {
		// 	paddleX -= speed;
		// } else if (paddleX + PADDLE_W/2 < canvas.width - 40) {
		// 	paddleX += speed;
		// }
	}
